...
if (!sessionStorage.getItem('bergvlietComments')) {
  // Create seed comments
  const seedComments = [
    {
      comment: 'Good job on turning this one around!'
    }
  ];

  // Store seed comments in session storage
  sessionStorage.setItem('bergvlietComments', JSON.stringify(seedComments));
}


// Check if there are any comments stored in session storage
if (sessionStorage.getItem('bergvlietComments')) {
  const comments = JSON.parse(sessionStorage.getItem('bergvlietComments'));
...
document.getElementById('commentForm').addEventListener('submit', function(event) {
  ...
  const comments = sessionStorage.getItem('bergvlietComments')
    ? JSON.parse(sessionStorage.getItem('bergvlietComments'))
    : [];
  ...
  sessionStorage.setItem('bergvlietComments', JSON.stringify(comments));
